...
class TrafficFormerDataset(TrafficStatePointDataset):

    def __init__(self, config):
        self.type_short_path = config.get('type_short_path', 'hop')
        super().__init__(config)
        self.cache_file_name = os.path.join('./libcity/cache/dataset_cache/',
                                            'trafficformer_point_based_{}.npz'.format(self.parameters_str))
        # 修复时间间隔大于1小时的情况
    ...
    def _get_dtw(self):
        ...
        if not os.path.exists(cache_path):
            ...
            if self.points_per_day > 0:
                data_mean = np.mean(
                    [df[self.points_per_day * i: self.points_per_day * (i + 1)]
                     for i in range(df.shape[0] // self.points_per_day)], axis=0)
            else:
                # 如果计算出现问题，使用全部数据
                data_mean = np.mean(df, axis=0, keepdims=True)
