...
class TrafficStateEvaluator(AbstractEvaluator):
    ...
    def save_result(self, save_path, filename=None, filename_prefix=None):
        ...
        if filename is None:
            ...
            filename = datetime.datetime.now().strftime('%Y_%m_%d_%H_%M_%S') + '_' + \
                       self.config['model'] + '_' + self.config['dataset'] + '_' + prefix + self.mode
        ...
        if 'csv' in self.save_modes:
            ...
            avg_metrics = {}
            for metric in self.metrics:
                avg_metrics[metric] = dataframe[metric].mean()
            avg_str = ', '.join([f'{k}: {v:.6f}' for k, v in avg_metrics.items()])
            self._logger.info(f'Average metrics over {self.len_timeslots} steps: {avg_str}')
        return dataframe
