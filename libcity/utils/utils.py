...
def get_logger(config, name=None):
    ...
    # 获取根logger并配置它，这样所有子logger都会继承配置
    root_logger = logging.getLogger()
    
    # 清除根logger的handlers，避免重复
    ...
    file_handler = logging.FileHandler(logfilepath, mode='w')  # 明确指定写入模式
    file_handler.setFormatter(formatter)
    file_handler.setLevel(level)  # 设置handler级别

    console_formatter = logging.Formatter(
        '%(asctime)s - %(levelname)s - %(message)s')
    console_handler = logging.StreamHandler(sys.stdout)
    console_handler.setFormatter(console_formatter)
    console_handler.setLevel(level)  # 设置handler级别

    # 将handlers添加到根logger，这样所有子logger都能使用
