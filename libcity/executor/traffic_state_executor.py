...
class TrafficStateExecutor(AbstractExecutor):
    ...
    def load_model_with_initial_ckpt(self, initial_ckpt):
        ...
        checkpoint = torch.load(initial_ckpt, map_location=torch.device('cpu'))
        
        # 兼容两种格式: dict格式 (save_model_with_epoch) 和 tuple格式 (save_model)
        if isinstance(checkpoint, dict):
            model_state = checkpoint.get('model_state_dict', checkpoint)
        else:
            model_state = checkpoint[0]  # tuple格式: (model_state, optimizer_state)
    ...
    def evaluate(self, test_dataloader, save_result=True, mode='test'):
        """
        评估模型性能
        
        Args:
            test_dataloader: 数据加载器（可以是测试集或验证集）
            save_result: 是否保存结果到文件
            mode: 评估模式，'test' 或 'val'，用于区分文件名
        
        Returns:
            评估结果字典
        """
        self._logger.info(f'Start evaluating on {mode} set...')
        with torch.no_grad():
            self.model.eval()
            y_truths = []
            y_preds = []
            ...
            if save_result:
                ...
                filename = \
                    ...
                    + self.config['model'] + '_' + self.config['dataset'] + f'_{mode}_predictions.npz'
                np.savez_compressed(os.path.join(self.evaluate_res_dir, filename), **outputs)
            
            self.evaluator.clear()
            self.evaluator.collect({'y_true': torch.tensor(y_truths), 'y_pred': torch.tensor(y_preds)})
            result = self.evaluator.save_result(self.evaluate_res_dir, filename_prefix=mode)
            return result
    
    def evaluate_val(self, eval_dataloader):
        """
        在验证集上评估模型（用于超参数实验）
        
        Args:
            eval_dataloader: 验证集数据加载器
        
        Returns:
            验证集评估结果字典
        """
        return self.evaluate(eval_dataloader, save_result=True, mode='val')
    ...
    def train(self, train_dataloader, eval_dataloader):
        ...
        # Warmup 信息
        if self.lr_warmup_epoch > 0:
            self._logger.info(f'Using warmup: {self.lr_warmup_epoch} epochs, init_lr: {self.lr_warmup_init}')

        batches_seen = num_batches * self._epoch_num
        for epoch_idx in range(self._epoch_num, self.epochs):
            # Warmup 学习率调整
            if epoch_idx < self.lr_warmup_epoch:
                warmup_lr = self.lr_warmup_init + (self.learning_rate - self.lr_warmup_init) * epoch_idx / self.lr_warmup_epoch
            ...
            # Warmup 结束后才使用 lr_scheduler
            if self.lr_scheduler is not None and epoch_idx >= self.lr_warmup_epoch:
