...
class ReweightedHuberLoss(nn.Module):
    """
    Huber(delta) + 峰值加权：
    - 对 y_true > p 分位的样本乘以 alpha (>1) 权重；
    - 其余为 1。
    """
    def __init__(self, delta=2.0, peak_quantile=0.9, peak_weight=2.0, reduction='mean'):
        super().__init__()
        self.delta = float(delta)
        self.peak_q = float(peak_quantile)
        self.peak_w = float(peak_weight)
        self.reduction = reduction
        self.register_buffer('_cached_q', torch.tensor(0.0), persistent=False)
        self._has_q = False

    def forward(self, pred, target):
        # pred, target: (B, T, N, 1) 或兼容形状
        diff = pred - target
        abs_diff = diff.abs()
        huber = torch.where(
            abs_diff <= self.delta,
        ...
        # 估计阈值：按当前 batch 的经验分位（也可离线缓存全局阈值）
        with torch.no_grad():
            q = torch.quantile(target.detach().float().reshape(-1), self.peak_q)
            self._cached_q = q
            self._has_q = True

        weights = torch.ones_like(huber)
        weights = torch.where(target >= self._cached_q, torch.full_like(weights, self.peak_w), weights)
        loss = huber * weights
...
class TrafficFormerExecutor(TrafficStateExecutor):
    ...
    def __init__(self, config, model):
        ...
        peak_q = float(config.get('peak_quantile', 0.9))
        peak_w = float(config.get('peak_weight', 2.0))
        self.aux_weight = float(config.get('aux_loss_weight', 0.2))  # 深监督系数
        
        # 构造加权Huber损失函数
        self.criterion = ReweightedHuberLoss(delta=delta, peak_quantile=peak_q, peak_weight=peak_w)
    ...
    def _train_epoch(self, train_dataloader, epoch_idx, batches_seen=None, loss_func=None):
        ...
        for batch in train_dataloader:
            ...
            # [修改] 支持模型返回深监督输出
            out = self.model(batch, batch_lap_pos_enc)
            if isinstance(out, (list, tuple)) and len(out) == 2:
                y_predicted, aux_preds = out
            else:
                y_predicted, aux_preds = out, None
            
            # [修改] 使用新的加权Huber损失
            # 主损失：对预测值进行逆变换后计算损失
            y_true_scaled = self._scaler.inverse_transform(y_true[..., :self.output_dim])
            y_pred_scaled = self._scaler.inverse_transform(y_predicted[..., :self.output_dim])
            main_loss = self.criterion(y_pred_scaled, y_true_scaled)
            
            # 深监督损失
            aux_loss = 0.0
            if aux_preds is not None and len(aux_preds) > 0 and self.aux_weight > 0:
                for aux in aux_preds:
                    aux_scaled = self._scaler.inverse_transform(aux[..., :self.output_dim])
                    aux_loss = aux_loss + self.criterion(aux_scaled, y_true_scaled)
    ...
    def _valid_epoch(self, eval_dataloader, epoch_idx, batches_seen=None, loss_func=None):
        with torch.no_grad():
            ...
            for batch in eval_dataloader:
                ...
                # [修改] 支持模型返回深监督输出
                out = self.model(batch, self.lap_mx)
                if isinstance(out, (list, tuple)) and len(out) == 2:
                    y_predicted, aux_preds = out
                else:
                    y_predicted, aux_preds = out, None
